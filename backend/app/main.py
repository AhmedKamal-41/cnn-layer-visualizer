...
from app.models.loaders import preload_model
...
async def startup_event():
    ...
    # Log torch cache directory
    torch_cache_dir = torch.hub.get_dir()
    logger.info(f"Torch cache directory: {torch_cache_dir}")
    ...
    if settings.PRELOAD_MODELS:
        logger.info(f"Model preloading enabled: {settings.PRELOAD_MODELS}")
        ...
        logger.info(f"Model preloading complete ({len(model_ids_to_preload)} models)")
